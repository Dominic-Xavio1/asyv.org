...
export async function POST(request){
    try{
      ...
      const response = await pool.query(
        `INSERT INTO api_post (created_by,title,content,media_url,media_type) 
         VALUES($1,$2,$3,$4,$5) 
      ...
      return NextResponse.json({
        ...
        console.log("/api/post POST Error: ",e)
        return NextResponse.json(
          { error: "Failed to create post", details: e.message }, 
          { status: 500 }
        );
    }
}
export async function GET(){
  ...
  `;
    ...
    console.log("/api/post GET Error: ", e);
    return NextResponse.json(
      { error: "Failed to fetch posts", details: e.message }, 
      { status: 500 }
    );
  }
}
...
export async function DELETE(request){
  try {
    ...
    const response = await pool.query(
      `DELETE FROM api_post WHERE id = $1 RETURNING id`,
      [postId]
    );
    ...
    console.log("/api/post DELETE Error: ", e);
    return NextResponse.json(
      { error: "Failed to delete post", details: e.message }, 
      { status: 500 }
    );
  }
}