...
export function LoginForm({
  ...
  useEffect(() => {
    ...
    if (error === "EmailNotRegistered") {
      toast.error("This email is not registered in our system. Please sign up or contact support.", { duration: 6000 });
      setIsLoading(false);
    } else if (error) {
      toast.error(`Login failed: ${error}`, { duration: 5000 });
      setIsLoading(false);
    }
  }, [searchParams])
              ...
              <Field className="grid grid-cols-3 gap-2 sm:gap-3">
                ...
                >
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="h-4 w-4 sm:h-5 sm:w-5">
                    ...
                    {isLoading && <><Spinner className="size-8 sm:size-6 text-yellow-500 ml-2" /></>}
